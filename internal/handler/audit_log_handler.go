...
func (s *Server) registerAuditLogRoutes(mux *http.ServeMux) {
	mux.HandleFunc("POST /api/audits", s.createAuditLog)
	mux.HandleFunc("GET /api/audits", s.listAuditLogs)
	mux.HandleFunc("GET /api/audits/{id}", s.getAuditLog)
	mux.HandleFunc("DELETE /api/audits/{id}", s.deleteAuditLog)
}
...
func (s *Server) createAuditLog(w http.ResponseWriter, r *http.Request) {
	...
	a, err := s.svc.CreateAuditLog(model.AuditLog{
